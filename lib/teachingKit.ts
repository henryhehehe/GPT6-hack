import {suggestedMuseumInvestigations} from './museumInvestigations';
...
export function lessonKitHtml(world:World,minutes:30|45|60) {
  ...
  const investigations=suggestedMuseumInvestigations(undefined,world.museumObjectIds??[]);
  ...
  return documentHtml(`${world.title} — teaching kit`,
    ...
    ${investigations.length?`<h3>Ready-to-use museum investigations</h3>${p('Choose one optional eight-minute discussion within investigation time, rather than adding all of these to the lesson length. Use the reading bridge only when it matches the assigned text.')}${investigations.map(plan=>`<article><h3>${escapeHtml(plan.title)}</h3>${p(`${plan.topic} · ${plan.minutes} minutes`)}<ol>${plan.steps.map(step=>`<li><strong>${escapeHtml(step.title)} · ${step.minutes} min</strong>${p(step.prompt)}</li>`).join('')}</ol>${p(`Return to the reading: ${plan.readingBridge}`)}</article>`).join('')}`:''}
...
export function learningReportHtml(world:World,students:Learner[],previewId:string|undefined,generatedAt:string,includePractice=false) {
  ...
  return documentHtml(`${world.title} — learning report`,
    `${p(`Downloaded: ${generatedAt}`)}${p(world.objective)}${p(`${joined.length} joined learners. ${practice?'Practice learner included separately.':'Teacher preview excluded.'} Snapshot of submitted work and saved museum notes; unsent drafts and paper responses are not included.`)}${p('Keep this report with your classroom records: it contains learner names and writing. AI-referenced source IDs alone are not proof of student-selected citations. Where recorded, exact selected quotes, source versions, predictions, and revision reflections are included below; older submissions may lack them.')}
    ${learners.length?learners.map(student=>{const summary=learnerSummary(student);return `<section><h2>${escapeHtml(student.id===previewId?'Your practice learner':student.name)}</h2>${p(`${student.turns.length} submissions · ${summary.status}`)}${student.museumNotes?.length?`<h3>Museum field notes</h3>${museumNotesHtml(student.museumNotes)}`:''}${student.prediction?`<h3>Starting prediction</h3>${p(student.prediction.text)}${p(`${student.prediction.at} · World v${student.prediction.worldVersion}`)}`:p('No starting prediction recorded.')}${student.archiveReflection?`<h3>Archive reflection</h3>${p(student.archiveReflection.text)}`:''}${student.turns.length?student.turns.map((turn,index)=>`<article><h3>Submission ${index+1}</h3>${p(`${turn.at} · World v${turn.worldVersion} · ${turn.scenario?'Hypothetical branch':'Baseline'}`)}<blockquote>${escapeHtml(turn.claim)}</blockquote>${turn.revisesTurnId?`${p(`Revises submission ${student.turns.findIndex((prior,i)=>turnId(prior,i)===turn.revisesTurnId)+1} · ${turn.revisionChanged?'Changed wording':'Unchanged resubmission'}`)}${p(`Reason for revision: ${turn.reflection??''}`)}`:''}${turn.citations?.length?`<h4>Student-selected passages</h4>${turn.citations.map(c=>`<blockquote>${escapeHtml(c.quote)}</blockquote>${p(`Source ${c.evidenceId} · ${c.material} · UTF-16 range ${c.start}–${c.end} · Version ${c.sourceVersion}`)}${p(`Learner explanation: ${c.relevance}`)}`).join('')}`:p('No student-selected passages recorded.')}${p(`Provisional AI feedback: ${turn.result.reply}`)}<ul>${turn.result.items.map(item=>`<li>${escapeHtml(rubricName(world,item.key))}: ${item.earned?'AI marked supported':'AI suggested review'} — ${escapeHtml(item.reason)}${item.excerpt?` (learner excerpt: “${escapeHtml(item.excerpt)}”)`:''}</li>`).join('')}</ul>${p(`AI-referenced evidence IDs: ${turn.result.evidenceIds.join(', ')||'None'}`)}${p(`Next question: ${turn.result.nextQuestion}`)}</article>`).join(''):p('No explanation submitted yet.')}</section>`;}).join(''):p('No joined learners yet.')}`);
}

function museumNotesHtml(notes:StudentState['museumNotes']){return notes?.length?notes.map(note=>{const object=museumObjects.find(item=>item.id===note.objectId);return `<article><h3>${escapeHtml(object?.title??note.objectId)}</h3>${p('Learner observation and interpretation — not a museum quotation or assessed source card.')}${p(`Observed: ${note.observation}`)}${p(`Interpretation: ${note.interpretation||'Not added.'}`)}${p(`Question or uncertainty: ${note.question||'Not added.'}`)}${p(`Saved: ${note.updatedAt}`)}${object?`<small>${escapeHtml(museumCitation(object))}</small>`:''}</article>`;}).join(''):p('No saved museum notes yet.');}
...
export function investigationDownloadHtml(world:World,student:StudentState,draft:string,scenario:boolean,generatedAt:string) {
  ...
  return documentHtml(`${world.title} — my investigation`,
    ...
    <h2>My museum field notes</h2>${museumNotesHtml(student.museumNotes)}
