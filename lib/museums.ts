...
const commonFields={
 ...
 topic:z.enum(['Alexandria','The Odyssey','Jane Austen','Macbeth','The Tempest','Declaration of Independence','Frankenstein','Charles Dickens','Frederick Douglass','Seneca Falls']),connection:z.string(),prompt:z.string(),limits:z.string(),
};
...
export function normalizeClevelandRecord(value:unknown,editorial:Pick<MuseumObject,'topic'|'connection'|'prompt'|'limits'>,retrievedAt:string):MuseumObject{
 const record=z.object({id:z.number().int().positive(),accession_number:z.string(),title:z.string(),creation_date:z.string(),culture:z.array(z.string()),technique:z.string(),measurements:z.string().optional(),creators:z.array(z.object({description:z.string(),qualifier:z.string().nullable().optional()})).default([]),creditline:z.string(),url:z.string(),images:z.object({web:z.object({url:z.string()})}),share_license_status:z.literal('CC0'),updated_at:z.string()}).parse(value);
 return MuseumObjectSchema.parse({id:`cma-${record.id}`,provider:'Cleveland Museum of Art',providerId:record.id,accession:record.accession_number,title:record.title,date:record.creation_date,culture:record.culture.join('; '),medium:record.technique,dimensions:record.measurements||'Not specified in the API record',creator:record.creators.map(c=>[c.qualifier,c.description].filter(Boolean).join(' ')).join('; ')||'Maker not identified in this record',credit:record.creditline,recordUrl:record.url,imageUrl:record.images.web.url,license:record.share_license_status,licenseUrl:'https://creativecommons.org/publicdomain/zero/1.0/',retrievedAt,sourceUpdatedAt:record.updated_at,...editorial});
}
