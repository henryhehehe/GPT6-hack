...
export function createDialogueDraftStore(key:string,sourceIdentity:string,storage:()=>Storage){
 const initial:Snapshot={message:'',warning:''};let snapshot=initial,loaded=false;
 const listeners=new Set<()=>void>();
 const notify=()=>listeners.forEach(listener=>listener());
 function load(){
  if(loaded)return;loaded=true;
  try{
   const saved=storage().getItem(key+':v2');
   if(saved){
 ...
 function save(next:Snapshot){
  snapshot=next;
  try{storage().setItem(key+':v2',JSON.stringify({sourceIdentity,message:next.message,pending:next.pending}));}
 ...
 return {
  ...
  complete(sent:PendingQuestion){
   ...
   try{const saved=storage().getItem(key+':v2');if(saved&&JSON.parse(saved).sourceIdentity!==sourceIdentity)return;}catch{}
