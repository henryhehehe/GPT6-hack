...
export async function astra<T>(name:string,schema:z.ZodType<T>,instructions:string,input:unknown){
 ...
 const res=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model:serverEnv('OPENAI_MODEL')||'gpt-6-astra',reasoning:{effort:'low'},instructions,input:JSON.stringify(input),max_output_tokens:5000,text:{format:{type:'json_schema',name,strict:true,schema:jsonSchema}}}),signal:AbortSignal.timeout(65000)});
