...
export async function astra<T>(name:string,schema:z.ZodType<T>,instructions:string,input:unknown,file?:{filename:string;file_data:string}|{file_url:string},timeoutMs=65000){
 ...
 const res=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model:serverEnv('OPENAI_MODEL')||'gpt-6-astra',reasoning:{effort:'low'},instructions,input:file?[{role:'user',content:[{type:'input_text',text:JSON.stringify(input)},{type:'input_file',...file}]}]:JSON.stringify(input),max_output_tokens:name==='complete_lesson'?12000:file?9000:7000,text:{format:{type:'json_schema',name,strict:true,schema:jsonSchema}}}),signal:AbortSignal.timeout(timeoutMs)});
