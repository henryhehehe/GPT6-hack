...
test('a new scene edit opens the teacher what-if even when the learner had chosen baseline',()=>{
 const storage=browserStorage(),oldKey=scenarioSelectionKey('class','learner','old-scene');
 saveScenarioSelection(storage,oldKey,false,true);
 const newKey=scenarioSelectionKey('class','learner','new-scene');
 assert.equal(readScenarioSelection(storage,newKey,true),true);
 saveScenarioSelection(storage,newKey,false,true);
 assert.equal(readScenarioSelection(storage,newKey,true),false);
 assert.equal(readScenarioSelection(storage,scenarioSelectionKey('class','other-learner','new-scene'),true),true);
});
