...
test('completion from a closed conversation preserves newer same-source writing after reopening',()=>{
 const {store}=fixture(),old=store();old.edit('Original question');const sent=old.prepare(()=> 'old');
 const reopened=store();reopened.read();reopened.edit('Newer reopened question');old.complete(sent);
 assert.equal(store().read().message,'Newer reopened question');
 const next=reopened.prepare(()=> 'new');old.complete(sent);
 assert.deepEqual(store().read().pending,next);
});
