...
export function loadTeachingCharacters(npcs:THREE.Group[]){
 ...
 for(const npc of npcs){void new GLTFLoader().loadAsync(`/models/characters/${files[npc.userData.character as ZoneId]}.glb`).then(gltf=>{
  ...
  const model=gltf.scene,bounds=new THREE.Box3().setFromObject(model),height=bounds.max.y-bounds.min.y,scale=1.8/Math.max(height,.01);model.scale.multiplyScalar(scale);model.position.y-=bounds.min.y*scale;
