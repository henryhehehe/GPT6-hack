...
export function surfaceTexel(finish:SurfaceFinish,u:number,v:number):{tone:number;height:number;roughness:number}{
 const grain=noise(u*73,v*71),broad=noise(u*7,v*9);
 let height=.55+grain*.15,tone=.94+grain*.06,roughness=.9;
 if(['brick','ashlar','slate'].includes(finish)){
  const cols=finish==='brick'?8:finish==='ashlar'?4:3,rows=finish==='brick'?16:finish==='ashlar'?8:5;
  const row=Math.floor(v*rows),x=u*cols+(row%2)*.5,y=v*rows;
  const edge=Math.min(fract(x),1-fract(x),fract(y),1-fract(y)),joint=1-smooth(.015,.065,edge),block=hash(Math.floor(x),row);
  height=(.53+grain*.10+block*.10)*(1-joint*.75);
  tone=.87+block*.11+grain*.02-joint*.18;roughness=finish==='slate'?.72:.88;
 }else if(finish==='planks'||finish==='parquet'){
  const cellX=Math.floor(u*4),cellY=Math.floor(v*4),turn=finish==='parquet'&&(cellX+cellY)%2===1;
  const x=turn?v:u,y=turn?u:v,board=x*12,end=y*(finish==='parquet'?4:2)+Math.floor(board)%2*.5;
  ...
  tone=.88+ring*.07+hash(Math.floor(board),Math.floor(end))*.05-joint*.2;height=.6+ring*.025-joint*.4;roughness=.56+grain*.12;
 }else if(finish==='sand'){
  const ripple=Math.sin(u*44+Math.sin(v*9)*1.8)*.5+.5;
  height=.4+ripple*.13+grain*.15;tone=.94+broad*.04+grain*.02;
 }else if(finish==='earth'){
  height=.35+broad*.25+grain*.2;tone=.87+broad*.1+grain*.03;
...
function maps(finish:SurfaceFinish){
 ...
 for(let y=0;y<SIZE;y++)for(let x=0;x<SIZE;x++){
  ...
  for(let c=0;c<3;c++){color[offset+c]=Math.round(p.tone*255);height[offset+c]=Math.round(p.height*255);rough[offset+c]=Math.round(p.roughness*255);}
 ...
 if(root&&profile){
  ...
  root.traverse(object=>{
   ...
   if(!material){
    ...
    material=old.clone();Object.assign(material,detail);material.bumpScale=finish==='limewash'?.012:finish==='planks'||finish==='parquet'?.02:.035;
