...
type Props={error?:string;saving?:boolean;settingImageUrl?:string|null;imported?:boolean;evidence:Evidence|null;collected:boolean;student:boolean;busy:boolean;onClose:()=>void;onCollect:()=>Promise<void>;onUse:(citation:Citation)=>void};
 ...
 return <Dialog open={!!evidence} onOpenChange={open=>{if(!open){onClose();setTab('text');}}}>
  <DialogContent className="evidence-reader">
   ...
   {tab==='text'||(imported&&!settingImageUrl)?<div className="reader-document">
    ...
    {evidence?.context&&<details><summary>Read in context</summary><p className="curriculum-context">{evidence.context.text}</p><small>{evidence.context.locator}. This is a bounded selection, not the complete chapter.</small></details>}
