...
export default function WorldScene(props:Props){
 ...
 useEffect(()=>{
  ...
  function animate(now:number){const dt=Number.isFinite(now-last)?Math.max(0,Math.min((now-last)/1000,.05)):0;last=now;t+=dt;const p=latest.current;
   ...
   const harbor=p.world.nodes.find(n=>n.id==='harbor')?.activity??.22,market=p.world.nodes.find(n=>n.id==='market')?.activity??.35;scenery.update(blend,harbor,market);details.update(blend,harbor,market);
