...
export default function WorldScene(props:Props){
 ...
 const teachingActors=useRef<ReturnType<typeof loadTeachingCharacters>|null>(null);
 const talk=(id:ZoneId)=>{teachingActors.current?.talk(id);props.onTalk(id);};
 ...
 useEffect(()=>{
  ...
  const teachingCharacters=loadTeachingCharacters(npcs);teachingActors.current=teachingCharacters;
  ...
  let measuredFrames=0,measuredAt=performance.now(),peakCalls=0,peakTriangles=0;let frame=0,t=0,last=performance.now(),blend=0,oldFocus:ZoneId|null=null,oldFocusRevision=props.focusRevision;const motionPreference=matchMedia('(prefers-reduced-motion: reduce)');
  ...
  function animate(now:number){const dt=Number.isFinite(now-last)?Math.max(0,Math.min((now-last)/1000,.05)):0;last=now;t+=dt;const reduced=motionPreference.matches,p=latest.current;
   ...
   blend=THREE.MathUtils.damp(blend,p.scenario?1:0,reduced?100:2,dt);atmosphere.update(t,reduced,appearance);teachingCharacters.update(dt,reduced,camera.position,explorer.walking);crowd.update(dt,t,camera,p.world,blend,reduced);district.upgradeFoliage(scenery.getPlantTemplate());
 ...
 return <><WorldAmbience worldId={'alexandria'}/>
  ...
  <div className="explorer-controls">
   ...
   <details className="character-directory"><summary>Talk to someone</summary>{(['harbor','market','library'] as ZoneId[]).map(id=><button key={id} onClick={()=>talk(id)}>{characters[id].name} · {characters[id].role}</button>)}</details>
   </div></details>
