...
export default function GeneratedWorldScene(props:Props){
 ...
 for(const [id,p] of Object.entries(spots) as [ZoneId,{x:number;z:number}][]){
  ...
  const guide=readingGuide(cast[id].color,{harbor:'#b78464',market:'#cfac87',library:'#8e624a'}[id],theme.landscape==='shore');guide.position.set(p.x+1.3,.22,p.z+.5);guide.userData.npc=id;scene.add(guide);characterAnchors[id]=guide;
 }
