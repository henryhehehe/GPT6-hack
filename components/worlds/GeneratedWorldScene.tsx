...
export default function GeneratedWorldScene(props:Props){
 ...
 const markers:THREE.Object3D[]=[],actors:THREE.Object3D[]=[];
 for(const [id,p] of Object.entries(spots) as [ZoneId,{x:number;z:number}][]){
  ...
  mesh(new THREE.BoxGeometry(7,.3,6),stone,p.x,4.7,p.z);
  const marker=mesh(new THREE.OctahedronGeometry(.45),gold,p.x-1.5,1.3,p.z+.5);marker.userData.zone=id;markers.push(marker);
  const body=mesh(new THREE.CapsuleGeometry(.3,.8,4,8),new THREE.MeshStandardMaterial({color:cast[id].color}),p.x+1.3,.95,p.z+.5);body.userData.npc=id;actors.push(body);const head=mesh(new THREE.SphereGeometry(.26,12,8),stone,p.x+1.3,1.75,p.z+.5);head.userData.npc=id;actors.push(head);
 }
 ...
 const ray=new THREE.Raycaster(),point=new THREE.Vector2();let down:[number,number]|null=null;const start=(e:PointerEvent)=>{down=[e.clientX,e.clientY];};const click=(e:PointerEvent)=>{if(!down||Math.hypot(e.clientX-down[0],e.clientY-down[1])>5)return;down=null;const rect=el.getBoundingClientRect();point.set((e.clientX-rect.left)/rect.width*2-1,-(e.clientY-rect.top)/rect.height*2+1);ray.setFromCamera(point,camera);const hit=ray.intersectObjects([...actors,...markers])[0];if(hit?.object.userData.npc)latest.current.onTalk(hit.object.userData.npc);else if(hit?.object.userData.zone)latest.current.onSelect(hit.object.userData.zone);else{const zone=external.inspect(ray);if(zone)latest.current.onSelect(zone);}};renderer.domElement.addEventListener('pointerdown',start);renderer.domElement.addEventListener('pointerup',click);
 const resize=new ResizeObserver(()=>{renderer.setSize(el.clientWidth,el.clientHeight);camera.aspect=el.clientWidth/Math.max(1,el.clientHeight);camera.updateProjectionMatrix();});resize.observe(el);let frame=0,last=performance.now(),oldRevision=-1;const v=new THREE.Vector3();
 function render(now:number){const dt=Math.max(0,Math.min((now-last)/1000,.05));last=now;const p=latest.current;if(oldRevision!==(p.focusRevision??0)){oldRevision=p.focusRevision??0;if(walk.walking)walk.focus(p.focus);else if(p.focus){const pos=spots[p.focus];orbit.target.set(pos.x,1,pos.z);camera.position.set(pos.x+12,12,pos.z+16);}else{orbit.target.set(0,0,-1);camera.position.set(34,31,43);}}walk.update(dt);if(!walk.walking)orbit.update();if(!reducedMotion.matches)markers.forEach(m=>{m.rotation.y+=dt*.5;});gold.color.set(p.scenario?'#f09857':'#dfbb72');for(const [id,pos] of Object.entries(spots) as [ZoneId,{x:number;z:number}][]){const button=labels.current[id];if(button){v.set(pos.x+1.3,2.8,pos.z+.5).project(camera);button.style.display=v.z>-1&&v.z<1&&Math.abs(v.x)<.95&&Math.abs(v.y)<.9?'flex':'none';button.style.left=`${(v.x+1)/2*el.clientWidth}px`;button.style.top=`${(1-v.y)/2*el.clientHeight}px`;}}renderer.render(scene,camera);frame=requestAnimationFrame(render);}frame=requestAnimationFrame(render);
