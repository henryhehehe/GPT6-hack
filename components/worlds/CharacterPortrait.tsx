...
export default function CharacterPortrait({access,world,npc}:{access:BuilderAccess|null;world:World;npc:ZoneId}){
 ...
 return <aside className="dialogue-portrait" aria-label={`Portrait of ${character.name}`}>
  {current?.url?<img src={current.url} alt={`AI-generated interpretation of ${character.name}, ${character.role}`}/>:<div className="portrait-placeholder" style={{'--portrait-accent':character.color} as React.CSSProperties}><span aria-hidden="true">{character.name[0]}</span><p role="status">{current?.status==='disabled'?'Explore this character’s perspective.':current?.status==='error'?'Your conversation is ready. The portrait is unavailable.':<><LoaderCircle size={16} className="spin"/> Preparing a portrait…<small>You can start talking now.</small></>}</p>{current?.status==='error'&&<button type="button" onClick={()=>setAttempt(n=>n+1)}><RefreshCw size={14}/> Retry portrait</button>}</div>}
