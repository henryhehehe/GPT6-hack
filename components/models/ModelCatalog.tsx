...
import {catalogLabel as label,catalogSize as size,readinessLabel,verifyCatalog,filterCatalog,catalogPlacements as placements,catalogSettings,catalogScenes,catalogWorldPlacements,sceneLabel,sceneSource,placementCode,loaderCode,placementBundle,type CatalogAsset,type CatalogPlacement} from '@/lib/modelCatalog';
...
function IntegrationPanel({asset,used}:{asset:CatalogAsset;used:CatalogPlacement[]}){
 ...
 return <section className="model-integrate" aria-label="Integration instructions">
  ...
  {ready&&placement?<>
   <label>Scene placement<select value={`${placement.setting}:${placement.key}`} onChange={e=>setKey(e.target.value)}>{used.map(p=><option key={`${p.setting}:${p.key}`} value={`${p.setting}:${p.key}`}>{sceneLabel(p.setting)} · {label(p.key)}</option>)}</select></label>
   {scene?.lessonId&&<a href={`/worlds?lesson=${encodeURIComponent(scene.lessonId)}`}>Explore this lesson scene →</a>}
   ...
   <p className="model-help">{example==='placement'?<>Edit this entry in <code>{sceneSource(placement.setting)}</code>. These coordinates are already resolved for this scene. For another instance, update keys and support references together; do not apply the theme translation twice. {placement.support&&`${placementBundle(placement).length} entries included so the supporting table or crate comes with it.`}</>:<>Use this when building a new scene. Existing lessons already load their art; keep their navigation, selection and cleanup hooks.</>}</p>
...
export default function ModelCatalog(){
 ...
 useEffect(()=>{const read=()=>{const id=new URL(window.location.href).searchParams.get('asset');if(id)setSelected(id);const scene=new URL(window.location.href).searchParams.get('scene');if(scene&&catalogSettings.includes(scene))setSetting(scene);};read();window.addEventListener('popstate',read);return()=>window.removeEventListener('popstate',read);},[]);
 ...
 const asset=filtered.find(a=>a.id===selected)??filtered[0],used=asset?placements.filter(p=>p.asset===asset.id&&(setting==='all'||p.setting===setting)):[];
 useEffect(()=>{if(!asset)return;const url=new URL(window.location.href);url.searchParams.set('asset',asset.id);if(setting==='all')url.searchParams.delete('scene');else url.searchParams.set('scene',setting);window.history.replaceState(window.history.state,'',url);},[asset,setting]);
 ...
 return <main className="model-catalog">
  ...
  <div className="model-catalog-body">
   <aside className="model-library" aria-label="Find models"><label htmlFor="model-search">Search models</label><input id="model-search" type="search" placeholder="Name, creator, purpose…" value={search} onChange={e=>setSearch(e.target.value)}/>
    <div className="model-filters"><label>Lesson scene / template<select value={setting} onChange={e=>setSetting(e.target.value)}><option value="all">All scenes and templates</option>{catalogScenes.map(s=><option value={s.id} key={s.id}>{s.label}</option>)}</select></label><label>Readiness<select value={status} onChange={e=>setStatus(e.target.value)}><option value="all">All models</option>{[...new Set(assets.map(a=>a.classroomStatus))].sort().map(s=><option key={s} value={s}>{readinessLabel(s)}</option>)}</select></label><label>Category<select value={category} onChange={e=>setCategory(e.target.value)}><option value="all">All categories</option>{[...new Set(assets.map(a=>a.category))].sort().map(c=><option key={c} value={c}>{label(c)}</option>)}</select></label><label>Sort<select value={sort} onChange={e=>setSort(e.target.value)}><option value="name">Name</option><option value="size">Smallest file first</option></select></label></div>
    ...
    <div className="model-list">{filtered.map(a=><button className={a.id===asset?.id?'selected':''} aria-pressed={a.id===asset?.id} key={a.id} onClick={()=>selectAsset(a.id)}><strong>{label(a.title)}</strong><span>{a.creator} · {size(a.bytes)}</span><small>{catalogWorldPlacements.some(p=>p.asset===a.id)?'In lesson':placements.some(p=>p.asset===a.id)?'Template only':a.classroomStatus==='scene-eligible'?'Available':'Reference only'} · {label(a.category)}</small></button>)}</div>
   </aside>
