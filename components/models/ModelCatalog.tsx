...
export default function ModelCatalog(){
  ...
  return <main className="model-catalog">
    <header className="model-catalog-header"><a href="/studio">← Teacher studio</a><span className="model-kicker">COUNTERFACTUAL WORLDS · ASSET LIBRARY</span><h1>Objects with a place and a purpose.</h1><p>69 local CC0 models, with provenance, measured files and guidance for reuse. This is a development reference; scene art does not establish historical facts.</p><div className="model-stats"><span><b>{assets.length||69}</b> catalog entries</span><span><b>{new Set(placements.map(p=>p.asset)).size}</b> models used in scenes</span><span><b>{placements.length}</b> placements across 4 settings</span></div></header>
