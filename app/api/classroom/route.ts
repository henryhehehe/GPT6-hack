...
export async function POST(request:Request){try{
 ...
 if(['scenario','author','director','apply'].includes(action)){
  ...
  if(action==='author'){
   ...
   const result=await astra('world',WorldSchema.omit({lessonPack:true}),boundary+' Build a compact harbor-market-library learning world from the supplied source text. Exactly one node per place. Keep all historical source evidence from the supplied template exactly unchanged; new content must be an assumption or teaching-prop, never a new historical source. Use the chosen intervention to create plausible, conditional consequences and visual activity levels 0..1. The geometry templates are fixed, but every explanation and activity level should follow the lesson. Keep concise.',{source,intervention,template:initialWorld});
