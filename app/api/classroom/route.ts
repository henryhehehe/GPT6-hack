...
import {characters,validateDialogue,dialogueHistory,dialogueInstructions} from '@/lib/characters';
...
export async function POST(request:Request){try{
 ...
 if(action==='talk'){
  ...
  const result=await astra('character_dialogue',DialogueSchema,boundary+' '+dialogueInstructions,{character:characters[npc],message,world,scenario,hint:state.hint,prior:dialogueHistory(student.dialogue??[],npc,scenario)});
