...
export async function POST(request:Request){try{
 ...
 if(action==='argue'){
  ...
  const evaluation={...gradeArgument(result.value,claim,student.evidence,world),responseId:result.responseId,latencyMs:result.latencyMs};student.turns.push({claim,npc,result:evaluation,at:new Date().toISOString(),worldVersion:row.version});student.unlocked ||= evaluation.unlocked;await saveStudent(sr,student);return json({ok:true,evaluation});
 }
