import {sceneDirectorSchema,applySceneChange,sceneInstructions} from '@/lib/sceneIntervention';
import {reserveAi,reserveClassAi,requireSameOrigin} from '@/lib/pilot';
import { zodToJsonSchema } from 'zod-to-json-schema';
import { z } from 'zod';
import { db,serverEnv,boundary } from '@/lib/server';
import { HintSchema } from '@/lib/world';
...
export async function GET(request:Request){
 ...
 server.addEventListener('message',async event=>{
  try{
   ...
   const context=await loadInterventionContext(db(),{id:msg.id,token:msg.token,studentId:msg.studentId,instruction:msg.instruction,sceneEdit:msg.sceneEdit});
   if(closed)return;
   const key=serverEnv('OPENAI_API_KEY');if(!key)throw new Error('Astra key is not configured');
   instruction=context.input.instruction;baseVersion=context.baseVersion;start=Date.now();
   ...
   const schema=zodToJsonSchema(context.input.sceneEdit?sceneDirectorSchema(context.input.world):HintSchema,{$refStrategy:'none'});delete schema.$schema;
   upstream.send(JSON.stringify({type:'response.create',model:serverEnv('OPENAI_MODEL')||'gpt-6-astra',reasoning:{effort:'medium'},instructions:boundary+interventionInstructions+(context.input.sceneEdit?sceneInstructions(context.input.world):''),input:JSON.stringify(context.input),max_output_tokens:4000,text:{format:{type:'json_schema',name:'intervention',strict:true,schema}}}));
