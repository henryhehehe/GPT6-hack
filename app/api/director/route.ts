...
export async function GET(request:Request){
 ...
 server.addEventListener('message',async event=>{
  try{
   ...
   if(!row||msg.token!==row.teacher_token)throw new Error('Only the teacher can direct this classroom');const {student:learner,basis}=await directorContext(db(),msg.id,msg.token,msg.studentId);const key=serverEnv('OPENAI_API_KEY');if(!key)throw new Error('Astra key is not configured');
   ...
   upstream.addEventListener('message',e=>{try{
    ...
    if(data.type==='response.completed'){
     ...
     if(!text)return;
     const value=HintSchema.parse(JSON.parse(text));send({type:'result',steered,result:{baseVersion,patch:{id:crypto.randomUUID(),...value,responseId:data.response.id,latencyMs:Date.now()-start,request:instruction,kind:'teaching-prop',basis}}});clearTimeout(timeout);close();
    }
   ...
   upstream.send(JSON.stringify({type:'response.create',model:serverEnv('OPENAI_MODEL')||'gpt-6-astra',reasoning:{effort:'medium'},instructions:boundary+' Create one teaching intervention with a concrete visual comparison and a guiding question. Do not reveal a complete answer or award points. Return concise copy for a prop in the world. Incorporate teacher corrections.',input:JSON.stringify({instruction,world:validateWorld(JSON.parse(row.world)),student:learner}),max_output_tokens:2500,text:{format:{type:'json_schema',name:'intervention',strict:true,schema}}}));
