...
if(assets.length!==69||new Set(assets.map((a)=>a.id)).size!==69)throw new Error('Expected 69 unique assets');
...
const budgets=(['alexandria','coast','garden','archive'] as ExternalSetting[]).map(setting=>{
 ...
 return{setting,placements:scene.length,uniqueAssets:ids.size,bytes:assets.filter((a)=>ids.has(a.id)).reduce((n:number,a)=>n+a.bytes,0)};
