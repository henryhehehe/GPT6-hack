...
const catalog=read('assets/external/catalog.json');
const assets=read('assets/external/runtime-catalog.json');
const placements=(['alexandria','coast','garden','archive'] as ExternalSetting[]).flatMap(setting=>externalPlacements(setting).map(p=>({...p,setting})));
if(assets.length!==69||new Set(assets.map((a:any)=>a.id)).size!==69)throw new Error('Expected 69 unique assets');
for(const asset of assets){
 const original=catalog.assets.find((a:any)=>a.id===asset.id);if(!original)throw new Error(`Missing original: ${asset.id}`);
 ...
 if((doc.buffers??[]).some((b:any)=>b.uri)||(doc.images??[]).some((i:any)=>i.uri))throw new Error(`External dependency: ${asset.id}`);
 asset.triangles=(doc.meshes??[]).reduce((sum:number,m:any)=>sum+m.primitives.reduce((n:number,p:any)=>{if(p.mode!==undefined&&p.mode!==4)throw new Error('Non-triangle primitive');return n+doc.accessors[p.indices??p.attributes.POSITION].count/3;},0),0);
 asset.materials=(doc.materials??[]).length;asset.skins=(doc.skins??[]).length;asset.clips=(doc.animations??[]).map((a:any)=>a.name);
...
for(const p of placements){const a=assets.find((a:any)=>a.id===p.asset);if(!a||a.classroomStatus!=='scene-eligible')throw new Error(`Ineligible placement: ${p.asset}`);}
for(const c of catalog.collections)c.runtime_integrated=assets.some((a:any)=>a.collection===c.id&&liveIds.has(a.id));
...
write('lib/externalAssetIndex.json',assets.map(({id,url,bytes,sha256,dimensions,category,classroomStatus}:any)=>({id,url,bytes,sha256,dimensions,category,classroomStatus})));
write('assets/external/placements.json',placements);
const budgets=(['alexandria','coast','garden','archive'] as ExternalSetting[]).map(setting=>{
 const scene=placements.filter(p=>p.setting===setting),ids=new Set(scene.map(p=>p.asset));
 return{setting,placements:scene.length,uniqueAssets:ids.size,bytes:assets.filter((a:any)=>ids.has(a.id)).reduce((n:number,a:any)=>n+a.bytes,0)};
});write('assets/external/scene-budgets.json',budgets);
const lines=['# External model usage guide','','Generated from source usage records, measured published GLBs, and the actual placement registry. Regenerate with `node --import tsx scripts/catalog-external-models.ts`.','','Open `/model-catalog` in the running application for search, one-at-a-time 3D previews, clip playback and downloads. All 69 entries have local self-contained GLBs; only reviewed static selections load in lessons.','',`Currently **${liveIds.size} distinct external models** appear in **${placements.length} placements across four settings**. Character bases/accessories and animation sources remain adaptation references. Scene-eligible means permitted for reviewed placement, not historically authenticated or device-benchmarked.`,'','## Scene payloads','','These are uncompressed GLB transfer bytes for external additions only, deduplicated within each scene. They exclude authored assets, JavaScript, renderer/GPU allocations, catalog metadata and textures after decoding. No FPS claim is implied.','','| Setting | Placements | Unique models | Extra GLB bytes |','| --- | ---: | ---: | ---: |',...budgets.map(b=>`| ${b.setting} | ${b.placements} | ${b.uniqueAssets} | ${b.bytes.toLocaleString()} |`),'','## Integration contract','','- Runtime placements live in `components/worlds/scene/externalLayout.ts`; only IDs from `lib/externalAssetIndex.json` are accepted. Never turn generated lesson text into a URL.','- Static exports use meters, Y-up, X/Z centered, and a ground origin. Place at scale 1. `Anchor_Inspect` is halfway up the bounds; `Anchor_Label` is above the top. The bounds describe the whole object, not the usable tabletop or hull waterline.','- Use `loadExternalModels(parent, placements)` for lessons. It deduplicates requests, allows three concurrent loads, shares static geometry/materials, and retains a box fallback on failure. Call `dispose()` before traversing the parent for cleanup. Late results are disposed instead of attached.','- `zone` opens the corresponding source station through the existing `onSelect` callback. It does not collect evidence, alter claim text, award points, or replace primary sources. Untagged objects are scenery.','- `solid` adds a conservative rotated ground footprint to generated-setting navigation. Small tabletop objects inherit their supporting furniture blocker. Alexandria uses separate navigation: update that registry when adding ground-level obstacles.','- Authored furniture, buildings, cave, sheep and teaching characters remain in place. External additions supplement them; generic ships/bodies do not replace reviewed period art.','- The public catalog intentionally permits reference previews of all assets; assembly and adaptation statuses are rejected by the classroom loader.','- Skinned references require `SkeletonUtils.clone`, one `AnimationMixer` per instance, and explicit retargeting/rest-pose/root-motion checks. Never use ordinary `Object3D.clone` for independent skeletons.','- The two animation GLBs are unmodified originals. Each contains 43 named clips including a T-pose. No retargeting or classroom animation acceptance is claimed.','- CC0 source links and saved license evidence are in `catalog.json` and `licenses/`. Keep credits and record modifications even though attribution is not a CC0 condition.','','## Item index','',...assets.map((a:any)=>`- [${a.title} — ${a.id}](#${a.id})`),''];
for(const a of assets){
 const used=placements.filter(p=>p.asset===a.id);
 lines.push(`<a id="${a.id}"></a>`,`## ${a.title}`,'',`**${a.id}** · ${a.category} · **${a.classroomStatus}**`,'',`[Local GLB](../../public${a.url}) · [Publisher / creator: ${a.creator}](${a.sourceUrl}) · [${a.license}](${a.licenseUrl})`,'',`- Retained source: \`${a.sourcePath}\``,`- Transfer: ${a.bytes.toLocaleString()} bytes; ${a.triangles.toLocaleString()} triangles; ${a.materials} materials; ${a.skins} skins.`,`- Dimensions X/Y/Z: ${a.dimensions.join(' / ')} m. ${a.anchors.length?'Static grounded/centered contract applies.':'Source transforms retained; fit and rescale deliberately.'}`,`- Anchors: ${a.anchors.join(', ')||'none added'}.`,'',`**Placement:** ${a.usage.placement}`,'',`**Review:** ${a.usage.cautions}`,'',...['loading','collision','interaction','animation','rights'].flatMap(k=>[`**${k[0].toUpperCase()+k.slice(1)}:** ${a.usage[k]}`,'']),'**Current use:**','',...(used.length?used.map(p=>`- ${p.setting} / \`${p.key}\`: position [${p.at.join(', ')}], yaw ${p.turn??0} rad; ${p.zone?'opens '+p.zone:'scenery only'}; ${p.solid?'solid ground footprint':'existing support or outside walking route'}.`):['Catalog preview only; no current lesson placement.']), '', '**How to reuse:**','');
 if(a.classroomStatus==='scene-eligible')lines.push('Add a reviewed entry to `externalLayout.ts`, choose a stable supporting surface and check the walking route. Do not copy the example coordinates blindly.','','```ts',`{ key: 'unique-name', asset: '${a.id}', at: [0, 0, 0], turn: 0 }`,'```','');
