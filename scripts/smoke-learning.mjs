...
async function post(body,token,status=200){const r=await fetch(base+'/api/classroom',{method:'POST',headers:{'Content-Type':'application/json',...(token?{Authorization:`Bearer ${token}`}:{})},body:JSON.stringify(body)});const text=await r.text();let d;try{d=JSON.parse(text);}catch{throw new Error(`Local service returned ${r.status} without JSON; this live gate is unverified.`);}assert.equal(r.status,status,JSON.stringify(d));return d;}
