...
import {museumObjects,normalizeClevelandRecord,normalizeMetRecord} from '../../lib/museums';
const candidate=[];
for(const item of museumObjects){
 const cleveland=item.provider==='Cleveland Museum of Art';
 const endpoint=cleveland?`https://openaccess-api.clevelandart.org/api/artworks/${item.providerId}`:`https://collectionapi.metmuseum.org/public/collection/v1/objects/${item.providerId}`;
 ...
 const updated=(cleveland?normalizeClevelandRecord:normalizeMetRecord)(cleveland?result.data:result,{topic:item.topic,connection:item.connection,prompt:item.prompt,limits:item.limits},new Date().toISOString().slice(0,10));
