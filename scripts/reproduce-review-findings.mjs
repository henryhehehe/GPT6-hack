...
assert.equal(createDialogueDraftStore('class:B:library:false','source1',()=>storage).read().message,'');
console.log('REPRODUCED: a late response from an earlier conversation instance erases newer saved typing after the dialog is reopened.');
...
try{
 ...
 assert.equal(otherLearner.request,instruction);assert.equal(otherLearner.basis.name,'Synthetic B');
 console.log('REPRODUCED: learner A receives the private teacher instruction and learner B basis in shared hint state.');
 pauseArgument=true;
 const assessing=action({...input,requestId:crypto.randomUUID(),claim:input.claim+' A new detail should be considered.'},b.studentToken,400);
 await pendingArgument;
 await action({action:'visit',id:c.id,studentId:b.studentId,zone:'harbor'},b.studentToken);
 unblock();const failed=await assessing;assert.match(failed.error,/progress changed/);
 assert.equal((await read(b)).student.turns.length,1);
 console.log('REPRODUCED: visit during an in-flight assessment discards the completed response with a progress-conflict error. Three local transport fixtures were consumed; only the first answer is saved.');
