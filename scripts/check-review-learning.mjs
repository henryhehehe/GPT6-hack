...
try{
 ...
 assert.equal(calls[1].input.student.name,undefined,'Shared hint context omits learner names');assert.equal(hint.patch.basis.name,'Synthetic B');assert.equal(calls[1].input.student.turns[0].claim,input.claim);
